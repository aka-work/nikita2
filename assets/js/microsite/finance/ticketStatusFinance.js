...
$('#btnFilter_mct').on('click', function() {
    ...
    let market = ''
    let checkout = $('#booking_mct').val()

    $.ajax({
        type: "POST",
        url: "/ticket/ticket-status/filter-myticket",
        data: { hotel, city, priority, division, create_from, create_to, created_by, pic, status: status.length > 0 ? JSON.stringify(status) : '', topic, validity, operator, booking, operator1, subdivision, country, shownotif, market, checkout },
        beforeSend: function() {
            let html = `<tr>
        ...
        success: function(data) {
            ...
            for (var i = 0; i < filter.length; i++) {
                ...
                html += `<tr onclick="window.open('/ticket/detail-ticket/${filter[i].id}','_blank');">
                                <td>${ no++}</td>
                                <td>
                                    <span style="display: none;">${moment(filter[i].utc_create).format('MM')}</span> ${moment(filter[i].utc_create).format('DD') + ' '} ${moment(filter[i].utc_create).format('MMM') + ' '} ${moment(filter[i].utc_create).format('YY')}
...
$('#btnFilter_overdue').on('click', function() {
    ...
    let market = ''
    let checkout = $('#booking_ov').val()

    $.ajax({
        type: "POST",
        url: "/ticket/ticket-status/filter-overdue",
        data: { hotel, city, priority, division, create_from, create_to, created_by, pic, status: status.length > 0 ? JSON.stringify(status) : '', topic, validity, operator, booking, operator1, subdivision, country, shownotif, market },
        beforeSend: function() {
            let html = `<tr>
                                <td colspan='12' align='center'>
                                    <img src='/assets/images/loading.gif' width='30' />
